        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (comboBox1.SelectedItem != null)
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    {
                        string tk = row["tk"].ToString(); // Thay "Tên tài khoản" bằng tên cột tài khoản trong bảng Excel
                        string mk = row["mk"].ToString(); // Thay "Mật khẩu" bằng tên cột mật khẩu trong bảng Excel
                        string tb = Environment.MachineName;

                        // Update mật khẩu vào cơ sở dữ liệu
                        string updateQuery = "UPDATE excel SET mk = @Password and tb = @tb WHERE tk = @Username"; // Thay "YourTable", "Password", "Username" bằng tên bảng và tên cột tương ứng trong cơ sở dữ liệu của bạn
                    ...
                    MessageBox.Show("Cập nhật mật khẩu thành công!", "Trạng Thái");
                }
                Application.Exit();
            }
        }
